...
class _ShopeeScreenState extends State<ShopeeScreen> {
  ...
  final double _acrescimoComissaoFreteGratis = 0.06; // 6% se participa do Prog. Frete Grátis
  final double _taxaFixaBaseShopee = 3.00; // R$3 por item (verificar valor atual)
  final double _tetoComissaoShopee = 100.00; // R$100 de teto na comissão percentual
  ...
  void _calcularPreco() {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      double comissaoShopeeEfetiva = _comissaoShopeePercentualBase + (_participaFreteGratis ? _acrescimoComissaoFreteGratis : 0);
      ...
      double denominadorBase = 1.0 - comissaoShopeeEfetiva - _lucroPercentualDesejado;
      if (denominadorBase <= 0) {
        _observacaoCalculo = "Erro: Comissão + Lucro somam 100% ou mais.";
        setState(() {});
        return;
      }
      double pvPreliminar = (custoProduto + custoFrete + _taxaFixaBaseShopee) / denominadorBase;
      ...
      if (valorComissaoPercentualCalculada > _tetoComissaoShopee) {
        ...
        if (denominadorComTeto <= 0) { /* Erro */ _observacaoCalculo = "Erro: Lucro muito alto."; setState((){}); return; }
        pvPreliminar = (custoProduto + custoFrete + _taxaFixaBaseShopee + _tetoComissaoShopee) / denominadorComTeto;
      }
      ...
      if (pvCalculado < 8.00 && pvCalculado > 0) {
        _observacaoCalculo = (tetoAplicado ? "Teto de comissão aplicado. " : "") +
            "ATENÇÃO: PV < R\$8. Verifique as políticas da Shopee para taxas em itens de baixo valor (pode haver alteração na taxa fixa de R\$${_taxaFixaBaseShopee.toStringAsFixed(2)}).";
      } else if (tetoAplicado) {
        _observacaoCalculo = "Teto de comissão de R\$${_tetoComissaoShopee.toStringAsFixed(2)} aplicado.";
      } else {
      ...
      _totalTaxasPlataforma = valorComissaoPercentualCalculada + taxaFixaAplicada;
      ...
      child: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: <Widget>[
              Text("Calculadora Shopee", style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold, color: Colors.teal.shade800)),
              ...
              SwitchListTile(
                ...
                subtitle: Text("(+${(_acrescimoComissaoFreteGratis * 100).toStringAsFixed(0)}% de comissão)"),
              ...
              if (_observacaoCalculo.isNotEmpty)
                Padding(
                  ...
                  child: Text(_observacaoCalculo, style: TextStyle(color: _observacaoCalculo.startsWith("Erro") ? Colors.redAccent : Colors.orange.shade800, fontStyle: FontStyle.italic)),
                ),
