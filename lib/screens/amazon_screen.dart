...
class _AmazonScreenState extends State<AmazonScreen> {
  ...
  final TextEditingController _taxaMinimaController =
      TextEditingController(text: '1.00'); // R$
    ...
    if (_formKey.currentState!.validate()) {
      ...
      final double custoFrete =
          double.tryParse(_custoFreteController.text.replaceAll(',', '.')) ??
              0.0;
      final double comissaoInput = double.tryParse(
              _comissaoPercentualController.text.replaceAll(',', '.')) ??
          0.0;
      final double taxaMinimaInput =
          double.tryParse(_taxaMinimaController.text.replaceAll(',', '.')) ??
              0.0;

      if (comissaoInput <= 0 ||
          comissaoInput >= (100.0 - (_lucroPercentualDesejado * 100))) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Comissão % inválida ou muito alta. Deve ser > 0 e < ${(100 - (_lucroPercentualDesejado * 100)).toStringAsFixed(0)}%')),
        );
        return;
      }
      ...
      double denominadorComPercentual =
          1.0 - comissaoPercentual - _lucroPercentualDesejado;
      if (denominadorComPercentual <= 0) {
        _observacaoCalculo = "Erro: Comissão + Lucro somam 100% ou mais.";
        setState(() {});
        return;
      }
      double pvTempComPercentual =
          (custoProduto + custoFrete) / denominadorComPercentual;
      double comissaoCalculadaPercentual =
          pvTempComPercentual * comissaoPercentual;

      // Cenário 2: Verificar se a taxa mínima é maior (e se foi informada)
      if (taxaMinimaInput > 0 &&
          comissaoCalculadaPercentual < taxaMinimaInput) {
        ...
        if (denominadorComTaxaMinima <= 0) {
          /* Erro */ _observacaoCalculo = "Erro: Lucro muito alto.";
          setState(() {});
          return;
        }
        pvCalculado = (custoProduto + custoFrete + taxaMinimaInput) /
            denominadorComTaxaMinima;
        comissaoValorAplicadaNaPlataforma = taxaMinimaInput;
        _observacaoCalculo =
            "Taxa mínima de R\$ ${taxaMinimaInput.toStringAsFixed(2)} aplicada.";
      } else {
        ...
        _observacaoCalculo =
            "Comissão percentual de ${comissaoInput.toStringAsFixed(0)}% aplicada.";
      }
      // --- FIM LÓGICA AMAZON ---
      ...
      child: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: <Widget>[
              Text("Calculadora Amazon Brasil",
                  style: Theme.of(context).textTheme.headlineSmall?.copyWith(
                      fontWeight: FontWeight.bold,
                      color: Colors.teal.shade800)),
              const SizedBox(height: 8),
              Text(
                  "Configure as taxas e custos para um lucro bruto de ${(_lucroPercentualDesejado * 100).toStringAsFixed(0)}% sobre o preço de venda.",
              ...
              _buildTextField(
                  ...
                  prefix: "R\$"),
              _buildTextField(
                  _comissaoPercentualController, "Comissão da Plataforma",
                  suffix: "%", isPercentage: true),
              _buildTextField(
                  _taxaMinimaController, "Taxa Mínima da Categoria (se houver)",
                  prefix: "R\$"),
              Padding(
  ...
  Widget _buildTextField(TextEditingController controller, String label,
      {String? prefix, String? suffix, bool isPercentage = false}) {
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        validator: (value) {
          ...
          if (isPercentage &&
              (numValue <= 0 ||
                  numValue >= (100 - (_lucroPercentualDesejado * 100)))) {
